...
const SidebarContent = ({
  ...
  useEffect(() => {
    var userType = authentication.user.usertype;
    ...
    if(!token){
      ...
      appRoute = appRoute.filter(data => data.path !== '/signup')
      if (userType === 'team-member' || userType === 'user' ) {
        appRoute = appRoute.filter(data => data.path !== '/data')
      }
    }
  ...
  const menu = (
    <Fragment>
      ...
      >
        ...
        {authentication.token !== null ? <Menu.Item
                onClick={() => deauthenticate()}
              >
                  <a>
                    {sidebarIcons && (
                      <span className="anticon"><LogOut strokeWidth={1} size={16}  /></span>
                    )}
                    ...
                    <a className="brand">
                      ...
                      >
                        {state.name}
								      ...
								      <img src="/images/logo.png" alt="green facilities ltd" style={{width: '100%'}} />
...
export default connect((state) => state, { deauthenticate })(withRouter(SidebarContent));
